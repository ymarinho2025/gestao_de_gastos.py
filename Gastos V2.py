#Controle de Gastos V2
#Pegar salario, ver futuros gastos, analisar quanto sobra, fazer grafico demonstrando no que mais vai o dinheiro
import matplotlib.pyplot as plt; import sys; import time
...
gastos = {}
finalizar = False # Inicializa a flag antes dos loops
...
# Linha 104 até 120 Cria o grafico e mostra ele
if gastos:
...
# ('n') pula linha (.join) Junta os itens (valor:.2f) deixa o valor em 2 casas decimais. Depois liste valores que quer que aparece na lista de gastos e feche tudo 
print('\n'.join([f"{nome}: R${valor:.2f}" for nome, valor in gastos.items()])) # Items só pode ser usados quando é dicionario

# Verifica se existem gastos
if gastos:
    ...
    # Isso permite mostrar mais de um gasto se houver empate
    ...
    perc_salario = (maior_valor / salario) * 100 if salario else 0 # O "if salario else 0" evita erro se o salário for zero

    # Verifica se há apenas um gasto com o maior valor
    if len(maiores_itens) == 1:
        # Mostra o nome do gasto, o valor com duas casas decimais e o percentual do salário
        ...
        # Caso haja empate, junta todos os nomes dos gastos empatados
        itens = ", ".join(f"'{n}'" for n in maiores_itens) # 'N' para nome
        # Mostra todos os gastos empatados, com o valor e percentual
